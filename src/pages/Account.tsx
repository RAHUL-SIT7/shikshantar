    ...
    <div className="flex flex-col gap-6 animate-in fade-in duration-500 max-w-5xl mx-auto mb-20">
      ...
      {totalDue > 0 ? (
        <div className="w-full bg-red-100 rounded-2xl p-6 flex flex-col md:flex-row items-center justify-between border border-red-200">
           ...
           <button className="bg-red-600 hover:bg-red-700 text-white px-6 py-3 rounded-xl font-bold shadow-md shadow-red-600/20 transition-colors whitespace-nowrap">
              📞 Contact Fee Office
           </button>
        </div>
      ) : (
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t border-gray-200 shadow-2xl md:hidden z-50">
         <a 
            href={`https://wa.me/9779800000000?text=${encodeURIComponent(`Namaste, Ma ${studentRecord.name} hu, Class ${studentRecord.class}, Mero fee barema kura garna chahanchhu.`)}`}
            target="_blank" rel="noopener noreferrer"
            className="w-full bg-[#1e3a8a] text-white py-3 rounded-xl font-bold flex items-center justify-center gap-2">
            📞 Contact Fee Office — 9800000000
         </a>
      </div>
                  ...
                  <div className="p-4 bg-blue-50 border border-blue-100 rounded-xl">
                     {receipt.months?.length > 0 && receipt.months.map((m: string) => (
                       <div key={m} className="flex justify-between text-sm mb-2 pb-2 border-b border-blue-100 last:border-0 last:mb-0 last:pb-0">
                         ...
                         <span className="text-blue-900 font-black">रू {((receipt.amount || 0) / receipt.months.length).toLocaleString()}</span>
                       </div>
                     ))}
